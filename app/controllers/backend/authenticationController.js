...
const handleEmailVerify = (req, res) => {
  ...
  jwt.verify(
    ...
    async (err, decoded) => {
      if (err) {
        return res.redirect(
          "https://deeplearndl.vercel.app/emailconfirm?error=nofound&id=1682208000-1687456800"
        );
      } else {
        ...
        if (userInfo.length !== 0 && userInfo[0].emailVerified) {
          return res.redirect(
            `https://deeplearndl.vercel.app/emailconfirm?success=done&id=${decoded.iat}-${decoded.exp}`
          );
          ...
          return res.redirect(
            `https://deeplearndl.vercel.app/emailconfirm?email=verify&id=${decoded.iat}-${decoded.exp}`
          );
        }
      }
    }
...
const handleTokenVerify = (req, res) => {
  ...
  jwt.verify(
    ...
    async (err, decoded) => {
      if (err) {
        return res.redirect("https://deeplearndl.vercel.app/forgot-error");
      } else {
        const userInfo = await userModel.find({ email: decoded.email });
        if (userInfo.length !== 0 && userInfo[0].emailVerified) {
          return res.redirect(
            `https://deeplearndl.vercel.app/change-password/${userInfo[0].forgotToken}`
          );
        }
      }
    }
...
const handleChangePassword = (req, res) => {
  ...
  jwt.verify(
    ...
    async (err, decoded) => {
      if (err) {
        return res.redirect("https://deeplearndl.vercel.app/login");
      } else {
        const userInfo = await userModel.find({ email: decoded.email });
